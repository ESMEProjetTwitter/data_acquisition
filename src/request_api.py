...
import pandas as pd
import numpy as np
url = "https://api.twitter.com/2/tweets/search/recent?query=concombre -is:retweet"
headers = {
...
comptage = "https://api.twitter.com/2/tweets/counts/recent?query=concombre&granularity=day"
...
with plt.style.context('dark_background') :
    ...
    plt.title("Nombre de Tweet sur les 7 derniers jours contenant le mot : concombre")
    plt.xlabel("Jour précédent la recherche")
    plt.ylabel("Nombre de Tweet")
    plt.show()

plt.hist(df['tweet_count'])
plt.title('nb de tweet')
plt.xlabel('jour')
plt.ylabel('Nombre de tweet')
plt.show()